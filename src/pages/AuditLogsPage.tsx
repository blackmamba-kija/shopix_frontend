...
import { Shield, Search, FileText, User, Activity, Calendar, LayoutDashboard } from "lucide-react";
...
const AuditLogsPage = () => {
    ...
    return (
        <AppLayout title={t("audit logs")} subtitle={t("track all administrative and security actions across the system")}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <Card className="bg-card border-none shadow-xl relative overflow-hidden group">
                        ...
                        <CardContent className="p-8">
                            <div className="flex items-center justify-between relative z-10">
                                <div>
                                    <p className="text-muted-foreground text-[10px] font-black uppercase tracking-widest">{t("active admins")}</p>
                        ...
                        <CardContent className="p-8">
                            <div className="flex items-center justify-between relative z-10">
                                <div>
                                    ...
                                    <h3 className="text-2xl font-black mt-3 tracking-tight text-foreground">{t("last 30 days")}</h3>
                                </div>
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow className="bg-secondary/10 hover:bg-secondary/10 border-b border-border/40">
                                            <TableHead className="w-[180px] font-black uppercase text-[10px] tracking-widest pl-6">{t("timestamp")}</TableHead>
                                        ...
                                        {filteredLogs.length === 0 ? (
                                            ...
                                            filteredLogs.map((log) => (
                                                <TableRow key={log.id} className="hover:bg-primary/5 transition-colors border-b border-border/40 last:border-0 group">
                                                    ...
                                                    <TableCell>
                                                        <Badge variant="outline" className="font-black text-[9px] uppercase tracking-wider py-1 px-2 border-primary/20 bg-primary/5 text-primary">
                                                            {log.action}
                                                        </Badge>
                                                    </TableCell>
                                                    ...
                                                    <TableCell className="text-sm font-medium text-muted-foreground max-w-sm py-4 pr-6">
                                                        {log.details}
                                                    </TableCell>
                                                </TableRow>
                                            ))
                                    ...
                                    <Card key={summary.name} className="border-none shadow-xl bg-card overflow-hidden group hover:shadow-2xl transition-all duration-300">
                                        <CardHeader className="flex flex-row items-center gap-5 pb-6 border-b border-border/40">
                                            ...
                                            <div className="flex-1 overflow-hidden">
                                                ...
                                                <CardDescription className="text-xs truncate font-bold text-muted-foreground mt-0.5">
                                                    {summary.email}
                                                </CardDescription>
                                            </div>
                                        </CardHeader>
                                        <CardContent className="p-6 space-y-6">
                                            <div className="flex justify-between items-center">
                                                <Badge className="capitalize font-black tracking-wider text-[10px] bg-secondary text-secondary-foreground hover:bg-secondary/80 border-none px-3 py-1">{t(summary.role)}</Badge>
                                                {summary.lastActive ? (
                                                    <span className="text-[10px] font-black uppercase text-muted-foreground flex items-center gap-1.5">
                                                        <Activity className="w-3 h-3 text-emerald-500" />
                                                        {t("active")}: {format(new Date(summary.lastActive), "MMM dd")}
                                                    </span>
                                                ) : (
                                            ...
                                            <div className="flex flex-wrap gap-1.5 pt-2">
                                                {Array.from(summary.modules).map((mod: any) => (
                                                    <Badge key={mod} variant="outline" className="text-[9px] font-black uppercase tracking-wider py-0.5 px-2 text-primary/70 border-primary/10">
                                                        {mod}
                                                    </Badge>
                                                ))}
                                            </div>
