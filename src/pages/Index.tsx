...
const Dashboard = () => {
  ...
  // ─── Which stat cards to show (at least one needed to render the row) ─────────
  const statCards = [
    { key: "dash_revenue",         title: t("total revenue"),    value: formatTsh(totalRevenue),                    icon: DollarSign,     change: t("today's combined"),         changeType: "positive" as const },
    { key: "dash_inventory_value", title: t("inventory value"),  value: formatTsh(inventoryValue),                  icon: Layers,         change: `${products.length} ${t("products")}`, changeType: "neutral"  as const },
    { key: "dash_profit",          title: t("est. profit"),      value: formatTsh(totalProfit),                     icon: TrendingUp,     change: t("today's estimate"),         changeType: "positive" as const },
    { key: "dash_alerts",          title: t("critical alerts"),  value: (lowStockProducts.length + outOfStockProducts.length).toString(), icon: AlertTriangle, change: t("items needing attention"), changeType: (lowStockProducts.length + outOfStockProducts.length > 0 ? "negative" : "neutral") as any },
  ].filter(c => show(c.key));

  // What chart panel to show
  const showWeeklyChart    = show("dash_weekly_chart");
  const showRevenueSources = show("dash_revenue_sources");
  const showInventoryPulse = show("dash_inventory_pulse");
  const showTopItems       = show("dash_top_items");
  ...
  return (
    <AppLayout title={t("dashboard")} subtitle={`${t("operational control center")} – ${user?.name || "Admin"}`}>
      <div className="space-y-8">
          ...
          <div className={cn(
            ...
            {statCards.map(c => (
              <StatCard
                ...
                className="bg-card shadow-xl border-none"
              />
            ))}
          </div>
        )}

        {/* ── Charts Row (Weekly + Revenue Sources) ───────────────────────── */}
        {(showWeeklyChart || showRevenueSources) && (
          <div className={cn(
            "grid gap-8",
            showWeeklyChart && showRevenueSources ? "grid-cols-1 lg:grid-cols-3" : "grid-cols-1"
          )}>

            {/* Weekly Performance Chart */}
            {showWeeklyChart && (
              <div className={cn("bg-card border-none shadow-2xl rounded-[2.5rem] p-4 group overflow-hidden relative", showRevenueSources ? "lg:col-span-2" : "")}>
                <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-full -mr-16 -mt-16 blur-2xl group-hover:bg-primary/10 transition-colors" />
                <div className="p-6 pb-2 relative z-10">
                  <div className="flex items-center gap-3 mb-8">
                    <div className="p-3 bg-primary/10 rounded-2xl"><BarChart3 className="w-5 h-5 text-primary" /></div>
                    <div>
                  ...
                  <ResponsiveContainer width="100%" height={340}>
                    <BarChart data={chartWeeklyData}>
                      ...
                      <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 11, fontWeight: 900, fill: "hsl(var(--muted-foreground))" }} tickFormatter={v => v >= 1000 ? `${(v / 1000).toFixed(1)}k` : String(v)} dx={-10} />
                      <Tooltip
            ...
            {showRevenueSources && (
              <div className="bg-card border-none shadow-2xl rounded-[2.5rem] p-8 relative overflow-hidden group">
                ...
                <div className="flex items-center gap-3 mb-10 relative z-10">
                  <div className="p-3 bg-primary/10 rounded-2xl"><PieChartIcon className="w-5 h-5 text-primary" /></div>
                  <div>
                ...
                <div className="relative flex justify-center py-4">
                  <ResponsiveContainer width="100%" height={240}>
                    <PieChart>
                      <Pie data={revenueSourceData} innerRadius={70} outerRadius={95} paddingAngle={10} dataKey="value" stroke="none">
                        {revenueSourceData.map((entry, i) => <Cell key={i} fill={entry.color} />)}
                      </Pie>
                      <Tooltip contentStyle={{ backgroundColor: "rgba(255,255,255,0.8)", backdropFilter: "blur(12px)", border: "none", borderRadius: "20px" }} formatter={(v: number) => formatTsh(v)} />
                    </PieChart>
                  </ResponsiveContainer>
                ...
                <div className="mt-10 space-y-4">
                  {[
                    { label: t("products"), pct: (productTotal / (productTotal + serviceTotal || 1)) * 100, solid: true },
                    { label: t("services"), pct: (serviceTotal / (productTotal + serviceTotal || 1)) * 100, solid: false },
                  ].map(row => (
                    <div key={row.label} className="flex items-center justify-between p-4 bg-secondary/30 rounded-2xl border border-border/40">
                      <div className="flex items-center gap-3">
                        <div className={cn("w-2 h-2 rounded-full bg-primary", !row.solid && "opacity-20")} />
                        <span className="text-xs font-black uppercase tracking-wider text-muted-foreground">{row.label}</span>
                      </div>
                      <span className="text-sm font-black italic">{row.pct.toFixed(1)}%</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {/* ── Inventory Pulse + Alerts + Top Items ────────────────────────── */}
        {(showInventoryPulse || showTopItems) && (
          <div className={cn(
            "grid gap-8",
            showInventoryPulse ? "grid-cols-1 lg:grid-cols-3" : "grid-cols-1"
          )}>
            ...
            {showInventoryPulse && (
              <div className="bg-card border-none shadow-2xl rounded-[2.5rem] p-8">
                <div className="flex items-center gap-3 mb-10">
                  <div className="p-3 bg-primary/10 rounded-2xl"><Package className="w-5 h-5 text-primary" /></div>
                  <div>
                    ...
                    <div key={item.name} className="space-y-3">
                      ...
                      <div className="h-4 w-full bg-secondary/50 rounded-2xl overflow-hidden shadow-inner">
                        <div
                          ...
                          style={{ width: `${(item.value / (products.length || 1)) * 100}%`, backgroundColor: item.color }}
                        >
                ...
                <div className="mt-12 p-6 bg-rose-500/5 border border-rose-500/10 rounded-2xl">
                  <div className="flex gap-3">
                    ...
                    <p className="text-[11px] text-rose-600 font-bold leading-relaxed italic lowercase">{t("low stock threshold warning")}</p>
                  </div>
                </div>
              </div>
            )}

            {/* Alerts + Top Items column */}
            {showTopItems && (
              <div className={cn("space-y-8", showInventoryPulse ? "lg:col-span-2" : "")}>
                <div className={cn("grid gap-8", "grid-cols-1 sm:grid-cols-2")}>
                    ...
                    <div className="bg-card border-none shadow-2xl rounded-[2.5rem] p-8">
                      ...
                      <div className="space-y-4">
                        {[...outOfStockProducts, ...lowStockProducts].slice(0, 4).map(p => (
                          <div key={p.id} className={cn("flex items-center gap-4 p-4 rounded-2xl border transition-all hover:scale-[1.02]", p.quantity === 0 ? "bg-rose-50/50 border-rose-100 dark:bg-rose-950/20 dark:border-rose-900/40" : "bg-warning/5 border-warning/10")}>
                            <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center shrink-0", p.quantity === 0 ? "bg-rose-100 text-rose-600 dark:bg-rose-950/50" : "bg-warning/20 text-warning")}>
                            ...
                            <div className="flex-1 overflow-hidden">
                              ...
                              <p className="text-[10px] font-bold text-muted-foreground mt-1 uppercase tracking-wider">
                                {p.quantity === 0 ? t("out of stock") : `${t("only")} ${p.quantity} ${t("remaining")}`}
                              </p>
                            </div>
                          </div>
                        ))}
                  ...
                  {/* Today's Top Items */}
                  <div className="bg-card border-none shadow-2xl rounded-[2.5rem] p-8 relative overflow-hidden">
                          ...
                          .map((item, i) => (
                            <div key={i} className="flex items-center justify-between p-4 bg-secondary/30 rounded-2xl border border-border/40 hover:bg-background transition-colors group">
                              <div className="flex items-center gap-4 overflow-hidden">
                                <div className="w-8 h-8 rounded-full bg-primary/5 border border-primary/10 flex items-center justify-center text-[10px] font-black group-hover:bg-primary group-hover:text-primary-foreground transition-all">
                                  0{i + 1}
                                </div>
        ...
        {/* ── Empty state for users with no dashboard permissions ─────────── */}
        {statCards.length === 0 && !showWeeklyChart && !showRevenueSources && !showInventoryPulse && !showTopItems && (
          <div className="flex flex-col items-center justify-center py-32 gap-6 opacity-30">
            <ShieldCheck className="w-24 h-24" />
            <p className="text-2xl font-black italic lowercase">{t("no dashboard widgets enabled")}</p>
            <p className="text-sm font-bold uppercase tracking-widest text-muted-foreground">{t("contact your admin to enable dashboard access")}</p>
          </div>
        )}
